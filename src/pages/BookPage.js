    ...
    if (Object.keys(dum[key1]['parts'][key2]['parts'])[0].startsWith('_')) {
     for (let key3 of Object.keys(dum[key1]['parts'][key2]['parts'])) {
      ...
      if (dum[key1]['parts'][key2]['parts'][key3]['read'])
       rwc2 += Math.round(dum[key1]['parts'][key2]['parts'][key3]['wc'])
     }
    } else {
     for (let key3 of Object.keys(dum[key1]['parts'][key2]['parts'])) {
      wc3 = 0
      ...
      for (let key4 of Object.keys(
       dum[key1]['parts'][key2]['parts'][key3]['parts']
      )) {
       wc3 += Math.round(
        dum[key1]['parts'][key2]['parts'][key3]['parts'][key4]['wc']
       )
       if (dum[key1]['parts'][key2]['parts'][key3]['parts'][key4]['read'])
        rwc3 += Math.round(
         dum[key1]['parts'][key2]['parts'][key3]['parts'][key4]['wc']
        )
      }
            ...
            for (let part1 of Object.keys(currentBook[part]['parts'])) {
             if (currentBook[part]['parts'][part1]['parts']) {
              for (let part2 of Object.keys(
               ...
               dumBook[part]['parts'][part1]['parts'][part2].read = markRead
                ? true
                : false
              }
             } else {
     ...
     ? Object.entries(currentBook).map(([partKey, partValue]) => {
        ...
        return (
         <IonItem
          onClick={() => {
           if (!markRead && !markUnread) {
            if (partKey.split('_').length - 1 > 1)
             history.push('/purports/' + partKey.replace('_', ''))
            else if (partKey.startsWith('_'))
             history.push('/purports/' + key + '.' + partKey.replace('_', ''))
            else if (
             !isNaN(keysList[keysList.length - 1]) &&
             !isNaN(partKey.replace('-', ''))
            )
         ...
         >
          ...
          {!partKey.startsWith('_') ? (
           <IonNote>
            {formatMinutes(Math.round(partValue.read_wc / wordsPerMin))} |{' '}
            {formatMinutes(Math.round(partValue.wc / wordsPerMin))} (
            {Math.floor((partValue.read_wc / partValue.wc) * 100)}
            %)
           </IonNote>
          ) : (
           <IonNote>
