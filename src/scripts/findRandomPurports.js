  ...
  for (const cantoIndex in booksData['SB']['parts']) {
   if (booksData['SB']['parts'][cantoIndex]['checked'] != 'false') {
    for (const chapterIndex in booksData['SB']['parts'][cantoIndex]['parts']) {
     if (
      booksData['SB']['parts'][cantoIndex]['parts'][chapterIndex]['checked'] !=
      'false'
     ) {
      for (const verse in booksData['SB']['parts'][cantoIndex]['parts'][
       chapterIndex
      ]['parts']) {
       purports.push([
        ...
        'Srimad Bhagavatam ' +
         ...
         '.' +
         verse.slice(1),
        booksData['SB']['parts'][cantoIndex]['parts'][chapterIndex]['parts'][
         verse
        ]['wc'],
       ])
      }
  ...
  for (const lila in booksData['CC']['parts']) {
   if (booksData['CC']['parts'][lila]['checked'] != 'false') {
    for (const chapterIndex in booksData['CC']['parts'][lila]['parts']) {
     if (
      booksData['CC']['parts'][lila]['parts'][chapterIndex]['checked'] !=
      'false'
     ) {
      for (const verse in booksData['CC']['parts'][lila]['parts'][chapterIndex][
       'parts'
      ]) {
       purports.push([
        ...
        'Caitanya Caritamrta ' +
         ...
         '.' +
         verse.slice(1),
        booksData['CC']['parts'][lila]['parts'][chapterIndex]['parts'][verse][
         'wc'
        ],
       ])
      }
...
function findRandomPurports(booksMap, vaniTime, wordsPerMin = 50) {
 ...
 while (true) {
  ...
  if (
   (wordsSumSoFar >= totalWords - wordsPerMin * 5 &&
    wordsSumSoFar <= totalWords) ||
   counter == 20
  ) {
   return purports
  }
