...
import { removeCircleOutline, chevronBackOutline } from 'ionicons/icons'
import {
 convertTo12HourFormat,
 formatMinutes,
} from '../scripts/durationToMinutes'
   ...
   for (let entry of Object.entries(userHistory[key])) {
    for (let i = 0; i < entry[1].length; i++) {
     entryList.push({
      ...
      index: i,
      duration:
       entry[1][i]['duration'] ?? Math.round(entry[1][i]['wc'] / wordsPerMin),
     })
    }
   }
  }
        ...
        setUserHistory((prev) => {
         ...
         for (let i = 0; i < entryList.length; i++) {
          if (entryList[i].isChecked) {
           ...
           if (dum[key][entryList[i].name].length == 0)
            delete dum[key][entryList[i].name]
          }
         }
         return dum
        })
      ...
      {entryList.map((entry) => {
       return (
        ...
        >
         ...
         <IonNote>
          {formatMinutes(entry.duration)} | {convertTo12HourFormat(entry.time)}
         </IonNote>
         {select ? (
          <IonCheckbox
