...
import { IonItem, IonButton, IonLabel, IonToolbar, IonButtons, IonCheckbox, IonPage, IonHeader, IonContent,
     IonRouterOutlet, IonIcon, IonReorderGroup, IonReorder, IonSegment, IonSegmentButton, IonCard, IonCardContent,
    IonFab, IonFabButton, IonFabList } from "@ionic/react";
import { Route, useRouteMatch, useHistory, useParams } from "react-router-dom";
import { Bookmarks, Settings } from '../context';
import { bookmarkOutline, removeCircleOutline, reorderThreeOutline, timeOutline, brushOutline,
   documentTextOutline, colorPaletteOutline, chevronBackOutline } from 'ionicons/icons';
...
function BookmarkList() {
 ...
 return (
     ...
     <IonPage>
      ...
      <IonToolbar>
      {settings.bookmark_type != "Read-Later" ? <IonButtons slot="start">
    <IonButton onClick={()=>{
            if(history.length > 1){
              history.goBack()
            }else{
    ...
    <IonIcon icon={chevronBackOutline}></IonIcon>
       ...
       <IonButtons slot="start">
        <IonLabel style={{ marginLeft: "10px" }}>{settings.bookmark_type == "Read-Later" ? key : key.slice(0, key.lastIndexOf("-"))}</IonLabel>
       </IonButtons>
    ...
    {reo ? 
      ...
      {bookmarksMap[key] ? bookmarksMap[key]["children"].map((child, i) => {
        let childName = child["type"] == "verse" ? child["name"].split(",")[0].replace(/_/g, " ") + (child["name"].split(",").length > 1 ? " to "+child["name"].split(",")[child["name"].split(",").length - 1].replace(/_/g, " ") : "") : child["name"].replace(/_/g, " ")
       if (!child["color"] || highlightColor == "all" || child["color"] == highlightColor) 
        return (
        <>
        {reo ? <IonReorder>
        <IonItem>
        {settings.bookmark_type == "highlights" || settings.bookmark_type == "notes" ? <IonCard
         style={ {"width": "90%"} }
        >
          <IonCardContent style={{textAlign:"justify"}}>
          {settings.bookmark_type == "highlights" ? child.text : child.notes}
           <IonLabel style={{textAlign:"right", marginTop: '10px'}}>{childName}</IonLabel> 
          </IonCardContent>
        </IonCard> :
         ...
         onClick={() => {
          if (!select){
            let focusElement = settings.bookmark_type == "highlights" || settings.bookmark_type == "notes" ? ("@" + (settings.bookmark_type == "notes" ? child.end_id + "*" + child.end_index : child.start_id + "*" + child.start_index)) : ""
            if(child.type == "lecture") history.push("/lecture/"+child.name+focusElement)
            if(child.type == "verse") history.push("/purports/"+child.name+focusElement)
          }
         }}
        >
        {settings.bookmark_type == "highlights" || settings.bookmark_type == "notes" ? <IonCard
         style={{"width": select ? "90%":"100%"}}
        >
          <IonCardContent style={{textAlign:"justify"}}>
            {settings.bookmark_type == "highlights" ? child.text : child.notes}
           <IonLabel style={{textAlign:"right", marginTop: '10px'}}>{childName}</IonLabel> 
      ...
      {settings.bookmark_type == "highlights" ?
          ...
          <IonFabButton >
            <div style={{backgroundColor: highlightColor != "all" ? highlightColor : settings.theme == "light" ? "#1e90ff" : "#3498db", borderRadius:"50%", height:"100%", width:"100%", display:'flex', alignItems:"center", justifyContent:"center"}}>
          ...
          <IonFabButton  onClick={()=>{setHighlightColor("all")}}>
            All
          </IonFabButton>
          </IonFabList>
          </IonFab>
      ...
      {settings.bookmark_type == "Read-Later" && bookmarksMap[key].children.length == 0 ? <p style={{ textAlign: "center", margin: "10px" }}>
        You can add lectures/purports to Read-Later by clicking <IonIcon icon={timeOutline}></IonIcon> Icon on the heading toolbar in their page
        </p> : null}
