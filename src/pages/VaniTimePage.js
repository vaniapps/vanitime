   ...
   <IonContent>
    ...
    >
     <div style={isPlatform('desktop') ? { width: '420px' } : {}}>
      ...
      >
       Scroll Vani Time: {formatVaniTime(vaniTime)}
      </div>
      ...
      >
       <IonButton onClick={getContent}>
        {contentModesMap[contentMode]['button']}
       </IonButton>
      </div>
           ...
           <AccordionItem>
            <AccordionHeader as={'div'}>
             ...
             >
              ...
              >
               <IonIcon icon={chevronDownSharp}></IonIcon> {bookValue.name}{' '}
              </IonLabel>

              <IonCheckbox
               indeterminate={bookValue.checked === 'partial'}
               style={{
                marginRight: '20px',
               }}
               checked={bookValue.checked === 'true'}
               onIonChange={(e) => {
                setBooksMap((prev) => {
                 let dum = {
                  ...prev,
            ...
            <AccordionBody>
             {alertsMap['accordion_body'] ? (
              Object.entries(bookValue.parts).map(([partsKey, partsValue]) => {
               return (
                <>
                 {Object.entries(partsValue.parts)[0][1]['parts'] ? (
                  <AccordionItem>
                   <AccordionHeader as={'div'}>
                    <div
                     style={{
                    ...
                    >
                     ...
                     <IonCheckbox
                      indeterminate={partsValue.checked === 'partial'}
                      style={{
                       marginRight: '40px',
                      }}
                      checked={partsValue.checked === 'true'}
                      onIonChange={(e) => {
                       setBooksMap((prev) => {
                        let dum = {
                         ...prev,
                        }
                        dum[bookKey]['parts'][partsKey]['checked'] = e.detail
                         .checked
                         ? 'true'
                         : 'false'
                        for (let part of Object.entries(
                         dum[bookKey]['parts'][partsKey]['parts']
                        )) {
                        ...
                        if (
                         count === Object.entries(dum[bookKey]['parts']).length
                        )
                         dum[bookKey]['checked'] = 'true'
                   ...
                   <AccordionBody>
                    {Object.entries(partsValue.parts).map(
                     ([subPartsKey, subPartsValue]) => {
                      return (
                       <div
                        style={{
                         width: '100%',
                       ...
                       >
                        ...
                        <IonCheckbox
                         ...
                         checked={subPartsValue.checked === 'true'}
                         onIonChange={(e) => {
                          setBooksMap((prev) => {
                           let dum = {
                            ...prev,
                           ...
                           dum[bookKey]['parts'][partsKey]['parts'][
                            subPartsKey
                           ]['checked'] = e.detail.checked ? 'true' : 'false'
                           let count = 0
                           for (let part of Object.entries(
                            dum[bookKey]['parts'][partsKey]['parts']
                           )) {
                           ...
                           if (
                            ...
                            dum[bookKey]['parts'][partsKey]['checked'] = 'true'
                           else if (count === 0)
                            dum[bookKey]['parts'][partsKey]['checked'] = 'false'
                           else
                            dum[bookKey]['parts'][partsKey]['checked'] =
                             'partial'
                           count = 0
                           for (let part of Object.entries(
                            dum[bookKey]['parts']
                           ...
                           if (
                            count ===
                            Object.entries(dum[bookKey]['parts']).length
                           )
                            dum[bookKey]['checked'] = 'true'
                           else if (count === 0)
                            dum[bookKey]['checked'] = 'false'
                           else dum[bookKey]['checked'] = 'partial'
                           return dum
                          })
                         }}
         ...
         >
          <IonCardHeader style={{ paddingBottom: '5px' }}>
           <IonCardTitle>
            <div style={{ textAlign: 'center' }}>
             {currentBook.name == 'OB'
              ? currentBook.part
              : booksMap[currentBook.name]['name']}
            </div>
           </IonCardTitle>
          ...
          <IonCardContent>
           <div style={{ textAlign: 'center' }}>
            {currentBook.name == 'BG' ? (
             <>
              Chapter: {currentBook.part} | Verse: {currentBook.verse}
             </>
            ) : null}
            {currentBook.name == 'SB' ? (
             <>
              Canto: {currentBook.part} | Chapter: {currentBook.sub_part} |
              Verse: {currentBook.verse}
             </>
            ) : null}
            {currentBook.name == 'CC' ? (
             <>
              Lila: {currentBook.part} | Chapter: {currentBook.sub_part} |
              Verse: {currentBook.verse}
             </>
            ) : null}
            {currentBook.name == 'OB' ? (
             <>Chapter: {currentBook.verse.replace(/_/g, ' ')}</>
            ) : null}
           </div>
          </IonCardContent>
         </IonCard>
    ...
    >
     ...
     <div className="wrapper">
      {alertsMap['sub_parts'] &&
      booksMap[tempCurrentBook['name']] &&
      booksMap[tempCurrentBook['name']]['parts'][tempCurrentBook['part']] &&
      Object.entries(
       booksMap[tempCurrentBook['name']]['parts'][tempCurrentBook['part']][
        'parts'
       ]
       ...
       <>
        {alertsMap['parts'] && tempCurrentBook['name'] ? (
         <IonRadioGroup
         ...
         >
          {Object.entries(booksMap[tempCurrentBook['name']]['parts']).map(
           ([partKey, partValue]) => {
            return (
             <IonItem>
        ...
        if (alertsMap['sub_parts']) {
         if (tempCurrentBook['sub_part'] === '') {
          ...
          setCurrentBook((prev) => {
           ...
           dum['verse'] = Object.keys(
            booksMap[dum['name']]['parts'][dum['part']]['parts'][
             dum['sub_part']
            ]['parts']
           )[0].replace('_', '')
           return dum
    ...
    >
     ...
     >
      Proceed with the below Lecture?
     </div>
     <div className="wrapper">
    ...
    >
     <div style={{ textAlign: 'center', margin: '10px' }}>
      Proceed with the below Purports?
     </div>
     <div className="wrapper">
      ...
      *You can change your reading speed in settings
     </div>
    </Modal>
    <IonToast
     onDidDismiss={() => {
