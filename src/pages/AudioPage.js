...
function Audio() {
 ...
 const [toastMessageMap, setToastMessageMap] = useState({
  ...
  loading_audio:
   'loading, please wait or check your internet connection or else try again later',
 ...
 const parseXmlToHtml = (xmlData) => {
  ...
  if (match && match[1] && matches.length == 1) {
   ...
   textContent = textContent.replace(
    /<p><br \/>\s*<audio[^>]*><source[^>]*src="([^"]+)".*?<\/audio>\s*<\/p>/,
    ''
   )
  }
  ...
  <IonPage>
   <IonHeader>
    <IonToolbar>
     ...
     <IonButtons
      style={{ width: '70%', margin: '0px', padding: '0' }}
      slot="start"
     >
      <IonLabel>
       {key
        .slice(key.indexOf('-_') + 2, key.lastIndexOf('-_'))
        .replace(/_/g, ' ')}
      </IonLabel>
     </IonButtons>
     <IonButtons slot="end">
   ...
   <IonContent className="ion-padding">
    ...
    >
     <div>
      <div style={{ textAlign: 'center', marginTop: '10px' }}>
       Audio Speed Rate{' '}
       {audi.current ? `(${audi.current.playbackRate}x)` : null}
      </div>
      <IonList>
       {[
    ...
    <IonFooter>
     ...
     >
      ...
      >
       ...
       >
        <IonIcon
         icon={!play ? playCircleOutline : pauseCircleOutline}
        ></IonIcon>
       </IonButton>
       <IonButton
        size="small"
       ...
       <div style={{ marginLeft: '5px' }}>
        {audi.current
         ? audi.current.duration && audi.current.duration != 'Infinity'
           ? formatTime(audi.current.duration.toFixed(0))
           : '0:00'
         : '0:00'}
       </div>
      </div>
