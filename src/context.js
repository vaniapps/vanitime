...
export function ContextProvider({ children }) {
 ...
 const [lecturesMap, setLecturesMap] = useLocal(
  'lectures-data',
  lecturesMapData
 )
 const [wordsPerMin, setWordsPerMin] = useLocal('words-per-min', 100)
 const [incompleteUserHistory, setIncompleteUserHistory] = useLocal(
  'incomplete-userhistory',
  ''
 )
 ...
 const [mediaFavoritesMap, setMediaFavoritesMap] = useLocal(
  'media-favorites',
  {}
 )
 const [lecturesTime, setLecturesTime] = useLocal('lectures-time', {
  heard: 0,
  total: 0,
 })
 const [checkAlerts, setCheckAlerts] = useLocal('check-alerts', {
  purports: false,
  lecture: false,
 })
 const [currentVersesMap, setCurrentVersesMap] = useLocal(
  'current-verses-map',
  {}
 )
    ...
    <Books.Provider value={[booksMap, setBooksMap]}>
     <WordsPerMin.Provider value={[wordsPerMin, setWordsPerMin]}>
      <IncompleteUserHistory.Provider
       value={[incompleteUserHistory, setIncompleteUserHistory]}
      >
           ...
           <LecturesTime.Provider value={[lecturesTime, setLecturesTime]}>
            <CheckAlerts.Provider value={[checkAlerts, setCheckAlerts]}>
             <CurrentVersesMap.Provider
              value={[currentVersesMap, setCurrentVersesMap]}
             >
               ...
               <Settings.Provider value={[settings, setSettings]}>
                <Goal.Provider value={[goal, setGoal]}>
                 <MediaFavorites.Provider
                  value={[mediaFavoritesMap, setMediaFavoritesMap]}
                 >
                  {children}
                 </MediaFavorites.Provider>
                </Goal.Provider>
