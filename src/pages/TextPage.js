...
IonToolbar, IonItem, IonLabel, IonCheckbox, IonButtons, IonButton, IonIcon, IonChip, IonSpinner, IonRadio, IonRadioGroup, 
IonInput, IonToast, IonFabButton, IonFab, IonFabList, IonTextarea } from '@ionic/react';
...
import { arrowDownOutline, arrowUpOutline, checkboxOutline, chevronBackOutline, bookmarkOutline, pencilOutline, colorPaletteOutline, folderOpenOutline, removeCircleOutline
,documentTextOutline, brushOutline, timeOutline } from 'ionicons/icons';
...
function Text(){
  ...
  const parseXmlToHtml = (xmlData, verse) => {
      ...
      if(key.startsWith("BG") || key.startsWith("SB") || key.startsWith("CC"))
        ...
        textContent = textContent.slice(textContent.indexOf("<p>"))
    ...
    const fetchData = async () => {
      try {
        ...
        const parsedHtmlContents = responses.map((response, i) => {
          ...
          return parseXmlToHtml(xmlData, Object.keys(versesMap)[i]);
        });
  ...
  useEffect(()=>{
    ...
    function onSelect(){
      ...
      if(selectedText){
        ...
        let selectedVerses = allVerses.slice(allVerses.indexOf(startVerse), allVerses.indexOf(endVerse)+endVerse.length)
  ...
  <IonButtons slot="end">
   <IonButton
    onClick={() => {
      setBookmarksMap((prev)=>{
        let dum = {...prev}
        dum["Read-Later"]["children"].push({"name": [...Object.keys(versesMap)].join(","),  "type": "verse", "isChecked": false})
        return dum 
      })
  ...
  <IonButtons slot="end">
   ...
   >
    ...
    <div style={{"fontSize":settings.font_size}}>
      ...
      <div className='content-container' dangerouslySetInnerHTML={{ __html: htmlContent }} /> 
    </div>
      ...
      <div style={{display:"flex", justifyContent:"space-evenly", marginTop:"10px"}}>
            ...
            <IonButton onClick={()=>{
                ...
                if(bookmarkType == "bookmarks") {
                  ...
                  setSettings(prev=>{
                    let dum = {...prev}
                    dum["bookmarks_folder"] = bookmarkInput["radio"]!="" ? bookmarkInput["radio"] : bookmarkInput["text"] + "-highlights"
                    return dum
                  })
