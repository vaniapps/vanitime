        ...
        if(keysList[0] == "year") {
            ...
            for (let typeKey of Object.keys(dumLectures)){
                for (let lecture of Object.keys(dumLectures[typeKey]['parts'])){
                    ...
                    if(!dumBook[year]["parts"][month]) dumBook[year]["parts"][month] = {"parts": {}}
                    dumBook[year]["parts"][month]["parts"][lecture] = dumLectures[typeKey]['parts'][lecture]
                }
            }
        ...
        for (let key1 of Object.keys(dum)) {
          ...
          if(Object.keys(dum[key1]['parts'])[0].includes("-_")){
            for (let key2 of Object.keys(dum[key1]['parts'])) {
                ...
                if(dum[key1]['parts'][key2]['read']) rtime2+=minutesToMinutes(dum[key1]['parts'][key2]['duration'])
            }
          }else{
            for (let key2 of Object.keys(dum[key1]['parts'])) {
                time3 = 0
                ...
                for (let key3 of Object.keys(dum[key1]['parts'][key2]['parts'])){
                    time3+= minutesToMinutes(dum[key1]['parts'][key2]['parts'][key3]['duration'])
                    if(dum[key1]['parts'][key2]['parts'][key3]['read']) rtime3+=minutesToMinutes(dum[key1]['parts'][key2]['parts'][key3]['duration'])
                }
                                            ...
                                            for (let part2 of Object.keys(currentBook[part]['parts'][part1]['parts'])) {
                                                for (let category of Object.entries(dum)) {
                                                    for (let lecture of Object.entries(category[1]["parts"])) {
                                                        if(lecture[0] == part2) {
                                                            console.log(part2)
                                                            dum[category[0]]["parts"][part2].read = markRead ? true : false 
                                                        }
                                                    }
                                                }
                                            }
                                    ...
                                    for (let category of Object.entries(dum)) {
                                        for (let lecture of Object.entries(category[1]["parts"])) {
                                            if(lecture[0] == part.slice(1)) {
                                                dum[category[0]]["parts"][part.slice(1)].read = markRead ? true : false 
                                            }
                                        }
                                    }
                                }
