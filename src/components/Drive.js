...
const CLIENT_ID =
 '16422091894-2aisb9q5f0qp4mc4u4v9di97hks1u0l8.apps.googleusercontent.com'
...
function Drive(props) {
  ...
  try {
   for (let i = 0; i < listResponse.result.files.length; i++) {
    ...
    if (listResponse.result.files[i].name == 'vanitimeappdata') {
     ...
     fileContent = await gapi.client.drive.files.get({
      alt: 'media',
      fileId: listResponse.result.files[i].id,
     })
 ...
 return (
  <div>
   ...
   <p>
    *Google needs to approve this app for users to login in this app. So It will
    take few days for this to function.
   </p>
   <Modal
   ...
   >
    <div style={{ margin: '10px' }}>
     ...
     {props.alertsMap['save'] ? (
      <div>
       You have last saved your data at {lastModified}. Would you like to
       overwrite that save?
      </div>
     ) : (
      <div>
       You have last saved your data at {lastModified}. Would you like to
       overwrite that current loacal data?
      </div>
     )}
     <div
      style={{
